  ...
  resource :articlelist do
    get do
      tags = Article.select("articles.title as title , a2.name as authorname , string_agg(t2.title, ',') as tagname")
                    ...
                    .joins("LEFT JOIN tags t2 ON t.tag_id = t2.id").where("t.deleted_at IS NULL").group(:title , :authorname)
      p = Axlsx::Package.new
  ...
  resource :taglist do
    get do
      tags = Tag.select("tags.title AS tagtitle , COUNT(a2.title) AS articlecount")
                ...
                .joins("LEFT JOIN articles a2 ON a.article_id = a2.id").where("a.deleted_at IS NULL").group(:tagtitle).order(articlecount: :desc)
      p = Axlsx::Package.new
